    ...
    const fetchFavoriteStatus = async () => {
      ...
      try {
        const response = await fetch(
          `/api/favorites/status?userId=${user.id}&propertyId=${id}`
        );
  ...
  const toggleFavorite = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          favoriteStatus ? "Favorite removed" : "Favorite added",
          data
        );
      } else {
        console.error(
    ...
    <div className="relative">
        ...
        <div className=" rounded-lg overflow-hidden">
          ...
          <div className="p-1 mt-1">
            ...
            <div className="flex-col text-gray-600">
              <p>{location}</p>
              <p className="text-black opacity-60">From ${price} / Night</p>
            </div>
          </div>
        </div>
      </Link>
      ...
      <button
        ...
        className={`absolute top-2 right-2 ${
          favoriteStatus ? "text-yellow-500" : "text-black-500"
        }`}
      >
        {favoriteStatus ? (
          <svg
